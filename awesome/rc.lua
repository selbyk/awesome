...
beautiful.init("/home/selby/.config/awesome/themes/default/theme.lua")
...
naughty.config.defaults.font = "PragmataPro 9"
...
tags = {
 ...
 layout = {
      layouts[1],  -- 1:float windows
      layouts[6],   -- 2:chat
      layouts[11],   -- 3:ide
      layouts[10],  -- 4:chrome
      layouts[11],   -- 5:text editor
...
-- menu icon menu pdq 07-02-2012
 ...
 local f = io.popen('ln -sfn ' .. home_path .. '.config/awesome/wallpaper/' .. wall .. ' ' .. home_path .. '.config/awesome/themes/default/arch.jpg')
 awesome.restart()
 end
...
vicious.register(weather, vicious.widgets.weather, "Weather: Baton Rouge, LA. Sky: ${sky}. Temp: ${tempc}c Humid: ${humid}%. Wind: ${windkmh} KM/h", 1200, "KBTR")
...
for s = 1, screen.count() do
   ...
   mywibox[s]:set_widget(layout)
    ...
    bottom_right_layout:add(volspace)
    bottom_right_layout:add(spacer)
...
for i = 1, keynumber do
	...
	awful.key({ }, "Scroll_Lock", function () awful.util.spawn("scrot -e 'mv $f ~/screenshots/ 2>/dev/null'") end)
	)
end
...
awful.rules.rules = {
    ...
    { rule = { class = "Sublime Text 2 (UNREGISTERED)" },
      properties = { tag = tags[1][5] } },
...
function run_once(prg,arg_string,screen)
    ...
    if not arg_string then 
        awful.util.spawn_with_shell("pgrep -f -u selby -x " .. prg .. "; or " .. prg .. "",screen)
    else
        awful.util.spawn_with_shell("pgrep -f -u selby -x " .. prg .. "; or " .. prg .. " " .. arg_string .. "",screen)
    end
end
awful.util.spawn_with_shell("/usr/lib/polkit-gnome/polkit-gnome-authentication-agent-1")
run_once("nm-applet", "--sm-disable")
--run_once("urxvt-tabbed","-tab-fg green -tab-bg black")
--run_once("hipchat", nil, 2)
